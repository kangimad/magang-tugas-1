@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between gap-x-5">
            <div>
                <p class="block text-sm text-gray-700 leading-5">
                    {!! __('Showing') !!}
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-200 bg-teal-800 border border-gray-300 cursor-default leading-5">{{ $page }}</span>
                                    </span>
                                @else
